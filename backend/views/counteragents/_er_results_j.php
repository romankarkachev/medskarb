...
<div class="card">
    <div class="card-block">
        <h3 class="card-title"><?= $details['shortName'] ?></h3>
        <p class="card-text"><strong>Полное наименование</strong>: <?= $details['name'] ?></p>
        <p class="card-text">
            <strong>ИНН</strong> <?= $details['inn'] ?>
            <strong>КПП</strong> <?= $details['kpp'] ?>
        ...
        <div class="alert alert-danger" role="alert">
            <strong>Закрыто <?= date('d.m.Y', strtotime($details['closeInfo']['date'])) ?></strong> <?= $details['closeInfo']['closeReason']['name'] ?>

        </div>
        <?php endif; ?>

        <?php if (isset($details['address'])): ?>
        <p class="card-text"><strong>Юридический адрес</strong>: <?= Counteragents::api_composeFullAddress($details['address']) ?></p>
        <?php endif; ?>

        <?php if (isset($details['email'])): ?>
        <p class="card-text"><strong>E-mail</strong>: <?= mb_strtolower($details['email']) ?></p>
        <?php endif; ?>

        <?php if (isset($details['authorizedCapital']['value'])): ?>
        <p class="card-text"><strong>Уставный капитал</strong>: <?= Yii::$app->formatter->asDecimal($details['authorizedCapital']['value'], 2) ?> р.</p>
        <?php endif; ?>
    ...
    <div class="card-footer text-muted">
        ...
        <?= $form->field($model, 'kpp')->hiddenInput()->label(false) ?>
